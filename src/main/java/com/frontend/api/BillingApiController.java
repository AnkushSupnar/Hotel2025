...
public class BillingApiController {
    ...
    /**
     * GET /api/billing/tables/sections
     * Get tables organized by section row groups (with merge info and sequence order).
     * Returns the same structure used by the desktop BillingController.loadSections().
     * Each row group is a list of sections that display side-by-side (merged).
     * Single-section groups display full-width.
     */
    @Operation(summary = "Get tables by section groups",
            description = "Returns tables organized by section row groups with merge and sequence info")
    @GetMapping("/tables/sections")
    public ResponseEntity<ApiResponse> getTablesBySection() {
        try {
            ...
            for (List<String> group : sectionGroups) {
                ...
                for (String section : group) {
                    ...
                    List<TableStatusDto> tableDtos = new ArrayList<>();
                    for (TableMaster table : tables) {
                        String status = calculateTableStatus(table.getId());
                        tableDtos.add(new TableStatusDto(
                                table.getId(),
                                table.getTableName(),
                                section,
                                status,
                    ...
                    java.util.Map<String, Object> sectionData = new java.util.LinkedHashMap<>();
                    sectionData.put("section", section);
                    sectionData.put("color", getMaterialColorForSection(section));
                    sectionData.put("tables", tableDtos);
                    groupData.add(sectionData);
                }
            ...
            LOG.info("Retrieved {} section groups", result.size());
            return ResponseEntity.ok(new ApiResponse("Section groups retrieved", true, result));
        } catch (Exception e) {
            LOG.error("Error retrieving section groups: {}", e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
    ...
    /**
     * Get Material Design color for a section name (same as desktop BillingController)
     */
    private String getMaterialColorForSection(String sectionName) {
        switch (sectionName.toUpperCase()) {
            case "A":  return "#1976D2";
            case "B":  return "#7B1FA2";
            case "C":  return "#C2185B";
            case "D":  return "#D32F2F";
            case "E":  return "#F57C00";
            case "G":  return "#388E3C";
            case "V":  return "#0097A7";
            case "P":  return "#5D4037";
            case "HP": return "#455A64";
            case "W":  return "#00796B";
    ...
    public ResponseEntity<ApiResponse> addItemsToTable(
        ...
        try {
            ...
            // KOT printing is NOT done here — matches desktop behavior where
            // adding items and printing KOT (Order button) are separate actions.
            // The user clicks the Order button to print KOT via /print-kot endpoint.
     ...
     * Generate KOT (Kitchen Order Ticket) PDF for items that haven't been sent to kitchen yet.
     * Returns PDF as Base64 for client-side download.
     * Only includes items with printQty > 0 and resets printQty after successful generation.
     */
    @Operation(summary = "Generate KOT PDF", description = "Generate Kitchen Order Ticket PDF for new/unprinted items on a table")
    ...
    public ResponseEntity<ApiResponse> printKOT(
        ...
        try {
            ...
            LOG.info("Generating KOT PDF for table {} with {} items", table.getTableName(), printableItems.size());

            // Generate KOT PDF bytes (in-memory, no printer involved)
            byte[] pdfBytes = kotOrderPrint.generateKOTPdfBytes(table.getTableName(), printableItems, waitorId);
            ...
            if (pdfBytes != null && pdfBytes.length > 0) {
                ...
                LOG.info("KOT PDF generated successfully for table {} - {} items",
                        table.getTableName(), printableItems.size());
                return ResponseEntity.ok(new ApiResponse("KOT generated successfully! " +
                        printableItems.size() + " items sent to kitchen.", true, result));
            } else {
                LOG.error("KOT PDF generation failed for table {}", table.getTableName());
                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                        .body(new ApiResponse("Failed to generate KOT PDF", false));
            }

        } catch (Exception e) {
            LOG.error("Error generating KOT for table {}: {}", tableId, e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse("Error generating KOT: " + e.getMessage(), false));
        }
    }
    ...
    public ResponseEntity<ApiResponse> getClosedBillForTable(@PathVariable Integer tableId) {
        try {
            ...
            // Explicitly load transactions to avoid LazyInitializationException
            // (matches desktop: loads transactions separately via getTransactionsForBill)
            List<Transaction> transactions = billService.getTransactionsForBill(closedBill.getBillNo());
            closedBill.setTransactions(transactions);
