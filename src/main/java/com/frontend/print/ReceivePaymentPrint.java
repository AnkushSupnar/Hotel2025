...
public class ReceivePaymentPrint {
    ...
    /**
     * Generate receipt PDF as byte array for web download.
     * Same layout as file-based version but writes to ByteArrayOutputStream.
     */
    public byte[] generateReceiptPdfBytes(SalesPaymentReceipt receipt, Customer customer) {
        if (receipt == null) return new byte[0];
        try {
            loadFonts();
            ...
            LOG.info("Receipt PDF bytes generated: ReceiptNo={}, size={}", receipt.getReceiptNo(), baos.size());
            return baos.toByteArray();
        } catch (Exception e) {
            LOG.error("Error generating receipt PDF bytes: {}", e.getMessage(), e);
            return new byte[0];
        }
    }
