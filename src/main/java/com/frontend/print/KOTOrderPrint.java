...
public class KOTOrderPrint {
    ...
    /**
     * Generate KOT PDF as byte array (for web download - no file written to disk)
     */
    public byte[] generateKOTPdfBytes(String tableName, List<TempTransaction> items, Integer waitorId) {
        if (items == null || items.isEmpty()) {
            LOG.warn("No items to generate KOT PDF for table {}", tableName);
            return null;
        }
        ...
        try {
            ...
            LOG.info("KOT PDF generated in memory for table {} ({} bytes)", tableName, baos.size());
            return baos.toByteArray();

        } catch (Exception e) {
            LOG.error("Error generating KOT PDF bytes for table {}: {}", tableName, e.getMessage(), e);
            return null;
        }
    }
